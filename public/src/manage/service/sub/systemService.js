        ...
        OverallGeneralSer.httpPostJsonData(OverallDataSer.urlData.getSystemManagePersonDataUrl, null,
            result => {
                ...
                for (let i in result.data) {
                    ManageDataSer.systemManage.statistic.person.data.push(result.data[i])
                }
                //解析数据结果进行排位
                let rankObj = {};
                for (let i in result.data) {
                ...
                //清空数组
                ManageDataSer.systemManage.statistic.person.append.rank.length=0;
                //循环添加rankObj到rank数组中
                for(let i in rankObj){
                    ManageDataSer.systemManage.statistic.person.append.rank.push(rankObj[i]);
                }
